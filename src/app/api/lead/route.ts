// "Zapytaj o wycenę" (product pages) — receives the form and passes it on.
...
export async function POST(request: Request) {
  ...
  if (!PHONE.test(lead.phone) || !EMAIL.test(lead.email) || !POSTCODE.test(lead.postcode) || !lead.consent) {
    return Response.json({ error: "invalid" }, { status: 400 });
  }
